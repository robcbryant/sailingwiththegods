...
public class Loan {
	...
	public Loan (float amount, float interestRate, float numOfDaysUntilDue, int settlementOfOrigin) {
		this.amount = amount;
	...
	public void AddNewSettlementToLog(int settlementID){
		...
		if (knownSettlements.Count > 0){
			...
			for(int i = 0; i <knownSettlements.Count; i++){
				if ( knownSettlements[i] == settlementID) break;//if we find a match break the loop and exit
...
public class CrewMember {
	...
	public int typeOfCrew; //0= sailor  1= warrior  2= slave  3= passenger 4= navigator 5= auger
	...
	public void AddOtherAttributes(script_player_controls playerShipVars, string captainsLog, PlayerRoute currentRoute){
		...
		Ship playerShip = playerShipVars.ship;
			...
			for (int index = 0; index < playerShip.crewRoster.Count; index++){
				...
				if(index < playerShip.crewRoster.Count - 1) CSVstring += "_";
			}
			...
			if (playerShip.currentLoan != null) CSVstring += "," + playerShip.currentLoan.amount;
			else CSVstring += ",-1";
...
public class globalVariables : MonoBehaviour {
	...
	//mat_water = (Material) GameObject.FindGameObjectWithTag("waterSurface").GetComponent<MeshRenderer>().sharedMaterial;
	...
	//void Update () {
		//float y = mat_waterCurrents.mainTextureOffset.y;
		//if(y > -1)
			//mat_waterCurrents.mainTextureOffset = new Vector2(0,mat_waterCurrents.mainTextureOffset.y - .01f);
		//else
			//mat_waterCurrents.mainTextureOffset = Vector2.zero;
	...
	void CreateSettlementsFromList(){
	...
	void LoadSettlementList() {
		...
		for (int lineCount = 1; lineCount < fileByLine.Length; lineCount++)
		{
			...
			Vector2 longXlatY;	try {longXlatY = new Vector2(float.Parse(records[3]), float.Parse(records[2])); } catch { longXlatY = Vector2.zero;}
			float elevation;	try {elevation = float.Parse(records[4]); } catch { elevation = 0f;}
			int population;		population = int.Parse(records[5]);
			...
			foreach(string networkID in parsedNetworks) settlement_masterList[lineCount-1].networks.Add(int.Parse(networkID));
	...
	void ShowConnectionsFromSettlement(Settlement currentSettlement){
	...
	public void SaveWaterCurrentZones(){
		...
		for (int currentZone = 0; currentZone < currentZoneParent.transform.childCount; currentZone++){
			...
			if (rowCounter == 40){rowCounter = 0; waterRoseData += "\n";}
	...
	public void CheckIfCurrentSettlementIsPartOfMainQuest(int settlementID){
		...
		//DEBUG
		//DEBUG_currentQuestLeg = playerShipVariables.ship.mainQuest.currentQuestSegment;
	}
	...
	//This function simply returns a predefined string based on the number value of the clout provided
		...
		else if(clout > 80 && clout <= 90)title   = "The Heroic";
		else if(clout > 90 && clout <= 99)title   = "The Legendary";
	...
	//	--or interactions with pirates and random events.
		...
		//###### First get the player's clout -- easy enough.
	...
	public void LoadMasterCrewRoster(){
		...
		//	Debug.Log(int.Parse(records[4]));
			...
			if (int.Parse(records[7]) == 1) isPartOfMainQuest = true;
	...
	public CrewMember GetCrewMemberFromID(int ID){
		foreach (CrewMember crewman in masterCrewList){
			if (crewman.ID == ID) return crewman;
		}
		return null;
	}
	...
	public void FillNewGameCrewRosterAvailability(){
		...
		foreach (CrewMember thisMember in masterCrewList){
			...
			if(newGameAvailableCrew.Count == 40) break;
	...
	public void InitiateMainQuestLineForPlayer(){
		...
		for (int i = 0; i < newGameAvailableCrew.Count; i++){
			if (newGameCrewSelectList[i]) playerShipVariables.ship.crewRoster.Add (newGameAvailableCrew[i]);
		}
	...
	public void AdjustPlayerClout(int cloutAdjustment){
		...
		if(GetCloutTitleEquivalency(clout) != GetCloutTitleEquivalency(clout + cloutAdjustment)){
			...
			Debug.Log ("Gained a level");
				ShowANotificationMessage( "Congratulations! You have reached a new level of influence! Before this day you were Jason, " + GetCloutTitleEquivalency(clout) + ".....But now...You have become Jason " + GetCloutTitleEquivalency(clout + cloutAdjustment) + "!");
			//If it was a decrease then show a negative message to the player
			} else {
			Debug.Log ("Lost a level");
				ShowANotificationMessage( "Unfortunately you sunk to a new low level of respect in the world! Before this day you were Jason, " + GetCloutTitleEquivalency(clout) + ".....But now...You have become Jason " + GetCloutTitleEquivalency(clout + cloutAdjustment) + "!");
			}
		}
		...
		if (playerShipVariables.ship.playerClout > 100) playerShipVariables.ship.playerClout = 100;
	...
	public void AdjustPlayerShipHealth(int healthAdjustment){
		...
		if (playerShipVariables.ship.health > 100) playerShipVariables.ship.health = 100;
		//if the health is reduced below 0 after the adjustment, then increase it back to 0
		if (playerShipVariables.ship.health < 0) playerShipVariables.ship.health = 0;
	}
	...
	public void SetupBeginnerGameDifficulty(){
		...
		if (gameDifficulty_Beginner) camera_Mapview.GetComponent<Camera>().enabled = true;
		else camera_Mapview.GetComponent<Camera>().enabled = false;
	}
	...
	public Network GetNetworkFromID(int ID){
		foreach (Network thisNetwork in masterNetworkList){
			if(ID == thisNetwork.ID) return thisNetwork;
		}
